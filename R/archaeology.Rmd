...
g1 <- g.b + 
  geom_point(data = s %>% arrange(maxAge), 
             aes(x = LONG,
                 y = LAT,
                 fill = cut(maxAge, c(3001, 2750, 2500, 2250, 2000, 1000, 0))),
             shape = 21, 
             size = 2, 
             color = 'grey',
             stroke = .25) + 
...
```{r}
p <- plot_grid(g3, p2, labels = "AUTO", ncol = 1, align = "v", rel_widths = c(1, 1), rel_heights = c(3, 1))
ggsave("../img//FigArch.pdf", p, width = 5, height = 8)
p
```
